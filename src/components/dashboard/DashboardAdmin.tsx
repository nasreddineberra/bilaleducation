...
const ABSENCE_TYPE: Record<string, string> = {
  absence: 'Absence',
  retard: 'Retard',
  authorized_absence: 'Abs. autorisée',
}
...
export default function DashboardAdmin({ stats, ...headerProps }: Props) {
  ...
  const collectPct = stats.totalDue > 0 ? Math.min(Math.round((stats.totalPercu / stats.totalDue) * 100), 100) : 0
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard title="Élèves actifs" value={stats.studentsActive} subtitle={`${stats.studentsTotal} au total`} icon={Users} iconBg="bg-primary-100" iconColor="text-primary-600" accentBar="bg-primary-500" />
        ...
        <StatCard title="Absences ce mois" value={stats.absencesThisMonth} subtitle={`${stats.absencesUnjustified} non justifiées`} icon={AlertTriangle} iconBg="bg-amber-100" iconColor="text-amber-600" accentBar="bg-amber-500" />
      </div>
          ...
          {stats.totalDue > 0 && (
            <div>
              <div className="h-3 bg-warm-100 rounded-full overflow-hidden">
                <div
                  className={clsx('h-full rounded-full transition-all', collectPct >= 80 ? 'bg-success-500' : collectPct >= 50 ? 'bg-amber-500' : 'bg-danger-500')}

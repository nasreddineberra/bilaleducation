...
- [x] Executer `supabase/migrations/guard-archived-period-writes.sql` : un bulletin archive
  ...
  **VERIFIE** : les 5 declencheurs poses et actifs, et les 5 noms cites par la purge correspondent
  bien a des declencheurs existants — un `ALTER TABLE ... DISABLE TRIGGER` ne se verifie qu'a
  l'execution, une faute de nom ne se serait revelee qu'a la cloture d'annee. Test utilisateur OK
  sur les gabarits.
- [x] Executer `supabase/migrations/guard-referentiel-delete.sql` : les 3 cles du referentiel
